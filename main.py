...
# ============================================================
def play_sound(path):
    """Reproduce un sonido sin bloquear la UI."""
    def _play(dt):
    ...
    def _schedule_alerts(self, name, apt_time):
        ...
        for alert_time, msg in alerts:
            ...
            if wait <= 0:
                continue
            time.sleep(wait)
            self._send_notification("📅 Recordatorio de Cita Médica", msg)
            play_sound(SND_NOTIFICATION)

        wait = (apt_time - datetime.now()).total_seconds()
        if wait > 0:
            time.sleep(wait)
            self._send_notification("📅 ¡Cita Médica Ahora!", f"Es la hora de tu cita: {name}")
            play_sound(SND_NOTIFICATION)
